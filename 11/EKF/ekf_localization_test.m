...
% 遷移行列Gtの計算
Gt = [1, 0, -vt/wt * cos(theta) + vt/wt * cos(theta + wt * dt);
...
% 位置と方位の予測ステップ
mu_tb = mu_t1 + [vt/wt * sin(theta + wt * dt) - vt/wt * sin(theta);
                -vt/wt * cos(theta + wt * dt) + vt/wt * cos(theta);
                wt * dt];
...
if (not(isempty(zt)))
    ...
    zth = [sqrt(q);
           atan2(m(2) - mu_tb(2), m(1) - mu_tb(1)) - mu_tb(3);
           0];

    % ヤコビアン行列Htの計算
    Ht = [(mu_tb(1) - m(1))/sqrt(q), (mu_tb(2) - m(2))/sqrt(q), 0;
          (m(2) - mu_tb(2))/q, (mu_tb(1) - m(1))/q, -1;
          0, 0, 0];
    ...
    % カルマンゲインKtの計算
    Kt = Sigma_tb * Ht' * inv(St);

    % 計測更新ステップ
    mu_tb = mu_tb + Kt * (zt - zth);
    Sigma_tb = (eye(3) - Kt * Ht) * Sigma_tb;
end
